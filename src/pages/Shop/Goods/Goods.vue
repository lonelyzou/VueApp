...
export default {
  ...
  data () {
    return {
      scrollY: 0, // 右侧列表滑动的坐标  在右侧滑动过程中更新这个值
      tops: [], // 左侧所有分类li的top值（clientHeight） 在列表初始显示之后更新一次即可
      food: {} // 需要显示的当前food
    }
  },
  computed: {
    ...
    currentIndex () {
      ...
      if (index !== this.index && this.leftScroll) {
        ...
        // 通过better-scroll库中的scrollToElement方法移动到找到的li相应位置
  ...
  watch: {
    ...
    goods () {
      this.$nextTick(() => { // goods列表显示了
        this._initScroll() // 更新scrollY
        this._initTops() // 更新右侧li的top值
      })
    }
  },
  methods: {
    ...
    showFood (food) {
      this.food = food
      this.$refs.food.toggleShow() // 通过找到组件标签，也就是组件对象，然后获取到子组件的方法
    }
  },
  components: {
    Food,
